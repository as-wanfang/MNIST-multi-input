...
def training_step(i, update_test_data, update_train_data):
    ...
    if update_test_data:
        id_test = np.argmax(mnist.test.labels, axis=1)
        batch_Y_test = np.stack([id_test>=5, id_test<5],axis=1).astype('float32')
        a, c, im = sess.run([accuracy, cross_entropy, It], {X: mnist.test.images, Y_: batch_Y_test})
