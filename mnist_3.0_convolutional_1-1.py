...
checkpoint_path = './checkpoint_1-1_n5'
...
# random divide into n nearly-equal-length partitions
def random_indicator(batch_id, n):
    return np.array(list(map(lambda x: [k for k, v in indicators.items() if x in v], batch_id)))
...
for i in range(10001):
    ...
    batch_id = np.argmax(batch_Y, axis=1)
    #batch_X2 = (batch_id>=5).astype('float32').reshape([100,1])
    batch_X2 = random_indicator(batch_id, n)
    # learning rate decay
    max_learning_rate = 0.003
    min_learning_rate = 0.0001
    decay_speed = 2000.0
    learning_rate = min_learning_rate + (max_learning_rate - min_learning_rate) * math.exp(-i/decay_speed)
    sess.run(train_step, {X: batch_X, X2: batch_X2, Y_: batch_Y, lr: learning_rate})
    # compute training values for visualisation
    if i%10==0:
        a = sess.run(accuracy, {X: batch_X, X2: batch_X2, Y_: batch_Y})
        print(str(i) + ": accuracy:" + str(a) + " (lr:" + str(learning_rate) + ")")
        a_train.append(a)
    # compute test values for visualisation
    if i%100==0:
        X2_test = random_indicator(np.argmax(mnist.test.labels, axis=1), n)
        a = sess.run(accuracy, {X: mnist.test.images, X2: X2_test, Y_: mnist.test.labels})
        a_test.append(a)
        X_test = mnist.test.images
        Y_test = mnist.test.labels
        y_ids = [] #[n, 10000, 10]
        for j in range(n):
            X2_test = np.tile(np.reshape(j, [1,1]),  [len(X_test), 1])
            y = sess.run(Y, {X: X_test, X2: X2_test})
            y_ids.append(y)
        test_prediction.append(np.array(y_ids))
        #y_ids = np.concatenate([y_ids[k] for k in range(n)], axis=1) #[10000, n*10]
        y_id = np.zeros([10000, 10])
        for k in range(0,n):
            y_id += y_ids[k]
        correct_prediction_ = np.equal(np.argmax(y_id, 1)%10, np.argmax(Y_test, 1)).astype('float32')
        a_true = np.mean(correct_prediction_)
        a_test_true.append(a_true)
        print(str(i) + ": ********* epoch " + str(i*100//mnist.train.images.shape[0]+1) + " ********* test accuracy:" + str(a) + " True test accuracy:" + str(a_true))

saver.save(sess, checkpoint_path + '/Network')

print("max test accuracy: " + str(max(a_test_true)))
np.savez(checkpoint_path+'/accuracy', a_test=a_test, a_train=a_train, a_test_true=a_test_true, test_prediction=test_prediction)
