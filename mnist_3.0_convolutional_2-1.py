...
def training_step(i, update_test_data, update_train_data):
    ...
    batch_Y = np.stack([batch_id>=5, batch_id<5],axis=1).astype('float32')

    id = np.array([i for i in range(0,10)])*10
    batch_id = np.tile(np.reshape(id,[1,10]),[100,1])
    batch_X2 = np.reshape(np.sum(np.multiply(batch_id, batch_Y), axis=1),[100,1])
    # learning rate decay
    ...
    if update_train_data:
        a, c, im, w, b = sess.run([accuracy, cross_entropy, I, allweights, allbiases], {X: batch_X, X2: batch_X2, Y_: batch_Y})
    ...
    if update_test_data:
        X2_test = np.reshape(np.sum(np.multiply(np.tile(np.reshape(id,[1,10]),[10000,1]), mnist.test.labels), axis=1), [10000,1])
        a, c, im = sess.run([accuracy, cross_entropy, It], {X: mnist.test.images, X2: X2_test, Y_: mnist.test.labels})
...
saver.save(sess, './checkpoint/Network')
